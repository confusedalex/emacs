...
(use-package corfu
  ...
  (corfu-auto-delay 0.2)    ;; lower delay for completion
...
(use-package elfeed-protocol
  ...
  (defun elfeed-protocol-fever-sync-unread-stat ()
    ...
    (elfeed-protocol-fever-reinit "fever+https://alex@rss.mytbu.de"))
  
  :custom
  (elfeed-use-curl t)
  ;; Change the db directory to a custom one
  ;; 'cause the default in "~./elfeed" and
  ;; I like a clean home directory :)
  (elfeed-db-directory "~/.emacs.d/elfeed")
  
  (elfeed-protocol-fever-update-unread-only t) ;; Only fetch unread articles
...
(with-eval-after-load 'evil
  (defun find-nearest-quote ()
    ...
    (let ((p (point))
      ...
      (if (or (not beg) (< (abs (- p beg)) (abs (- p end))))
          (progn
            (setq quote-char (char-to-string (char-after beg)))
            (setq end nil)
            (goto-char beg))
            (setq quote-char (char-to-string (char-after end)))
        (setq beg nil)
        (goto-char end))
      ))

    (defun goto-nearest-quote ()
      (interactive)
      (goto-char (find-nearest-quote)))

    (global-set-key (kbd "C-c t") 'find-nearest-quote)
    ) 
#+end_src
